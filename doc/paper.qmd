...
4. As in the original paper, firm-years are selected based on available stock return data in CRSP. While @ball_how_2008 restrict the sample to firms with at least 240 trading days per year, I follow the course guidance and do not apply this restriction, allowing for a slightly larger sample size. This adjustment should be considered when interpreting differences from the original study.

By following the steps outlined in @sec-replication_steps1 and adhering to the assumptions made, I successfully replicated Table 1 (Panel A), Table 2 (all panels), and Figure 1 (all panels) from @ball_how_2008. A thorough step-by-step approach, with each step clearly documented, helped to understand and verify the outputs.
